...
export default function ProductIntro({ product }: Props) {
  const { id, price, name, images, slug, thumbnail } = product || {};
  ...
  const handleCartAmountChange = (amount: number) => () => {
    dispatch({
      ...
      payload: { price, qty: amount, name: name, imgUrl: thumbnail, id, slug }
    });
  };
            ...
            <Box key={variant.id} mb={2}>
              ...
              {variant.values.map(({ id, value }) => (
                <Chip
                  key={id}
                  label={value}
                  onClick={handleChangeVariant(variant.name, value)}
                  sx={{ borderRadius: "4px", mr: 1, cursor: "pointer" }}
                  color={
                    selectVariants[variant.name.toLowerCase() as keyof typeof selectVariants] === value
                      ? "primary"
                      : "default"
                  }
                />
