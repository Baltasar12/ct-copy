...
export default function ProductCard14({ product, btnSmall = false }: Props) {
  const { id, slug, name, thumbnail, price, compare_at_price } = product || {};

  // Si compare_at_price es null, no hay descuento
  const discountPercentage = compare_at_price ? calculateDiscount(price, compare_at_price) : 0; 
    ...
    <Fragment>
      <Link href={`/products/${slug}`}>
        <FlexBox bgcolor="grey.50" borderRadius={3} mb={2}>
          <LazyImage alt={name} width={260} height={280} src={thumbnail} />
        </FlexBox>
      </Link>
     ...
     {/*    <Paragraph color="grey.600" fontWeight={600}>
        ...
        <PriceText>
            {calculateDiscount(price, discountPercentage) /* error en el discount */}
            {discountPercentage ? <span className="base-price">{currency(price)}</span> : null}
        </PriceText>
