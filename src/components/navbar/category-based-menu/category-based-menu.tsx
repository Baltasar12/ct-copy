...
export default function CategoryBasedMenu({ title, menuList }: Props) {
  ...
  // Explicitly define the type for the accumulator array
  const categories = menuList.reduce<string[]>(
    (prev, curr) => [...prev, curr.title],
    [] // Provide an initial value of type string[]
  );
    ...
    <Wrapper>
      ...
      <MenusContainer className="menu-list">
        <StyledCard>
          ...
          {subCategories && <ChildCategories categories={subCategories} />}
        </StyledCard>
      </MenusContainer>
    </Wrapper>
  );
